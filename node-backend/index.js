...
app.post("/users/createUser", (req, res) => {
    ...
    // Validate other fields here as needed

    const insertQuery = "INSERT INTO users (firstName, lastName, emailId, designation, dateOfBirth, companyId) VALUES (?, ?, ?, ?, ?, ?)";
    db.query(insertQuery, [firstName, lastName, emailId, designation, dateOfBirth, companyId], (error, result) => {
        if (error) {
            console.log(error);
  ...
  app.post('/companies/createCompany',(req,res)=>{
    res.status(200);
    res.setHeader('Content-Type', 'text/html');
    const {companyName, address, latitude, longitude,description} =
    req.body;

    const query = "INSERT INTO companies (`companyName`, `address`, `latitude`, `longitude`, `description`) VALUES (?,?,?,?,?);";

    db.query(query,[companyName, address, latitude, longitude,  description],(error,response) =>{
      if(error){
        console.log("error is : ",error);
        return;
      }
      console.log('new company is successfully created ',response);
...
app.put("/companies/editCompany/:id", (req, res) => {
    const { id } = req.params;
    const { companyName, address, latitude, longitude, description } = req.body;

    const query = "UPDATE companies SET `companyName` = ?, `address` = ?, `latitude` = ?, `longitude` = ?, `description` = ? WHERE (`companyId` = ?)";

    db.query(query, [companyName, address, latitude, longitude, description, id], (error, response) => {
        if (error) {
            console.error("Error occurred while updating company:", error);
            return res.status(500).send("Internal Server Error"); // Handle the error response
        }

        console.log("Company updated successfully:", response);
        res.status(200).send("Successfully Updated"); // Send the success response
    });
});
